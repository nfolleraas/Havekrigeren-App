    ...
    public class Database
    {
        ...
        private static T ConvertToObject<T>(Dictionary<string, object> dict) where T : new()
        {
            if (typeof(T) == typeof(Job))
            {
                ...
                dict.TryGetValue("startDate", out object startDateObj);
                string startDateStr = startDateObj as string ?? "";
                DateOnly startDate = DateOnly.ParseExact(startDateStr, "dd/MM-yyyy");

                dict.TryGetValue("endDate", out object endDateObj);
                string endDateStr = endDateObj as string ?? "";
                DateOnly endDate = DateOnly.ParseExact(endDateStr, "dd/MM-yyyy");

                dict.TryGetValue("notes", out object notesObj);
                string notes = notesObj as string ?? "";

                return (T)(object)new Job(contactName, address, phoneNumber, category, startDate, endDate, notes);
            }
