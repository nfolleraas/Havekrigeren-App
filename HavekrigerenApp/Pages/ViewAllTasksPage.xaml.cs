namespace HavekrigerenApp.Pages
{
    public partial class ViewAllTasksPage : ContentPage
    {
        ...
        private async void OnTaskClicked(TaskManager task)
        {
            var taskInfo = new TaskManager(task.ContactName, task.Address, task.PhoneNumber, task.Category, task.Date, task.Notes);

            await Navigation.PushAsync(new ViewTaskPage(taskInfo));
        }
    }
}